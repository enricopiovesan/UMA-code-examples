...
export async function formatGraphDiff(rootDir, fromScenario, toScenario) {
  const fromGraph = (await loadScenario(rootDir, fromScenario)).graph;
  const toGraph = (await loadScenario(rootDir, toScenario)).graph;

  const toKey = (edge) => `${edge.from}->${edge.event}->${edge.to}`;
  const toWaitingKey = (entry) => `${entry.service} waiting for ${entry.event}`;

  const fromEdges = new Set(fromGraph.edges.map(toKey));
  const toEdges = new Set(toGraph.edges.map(toKey));
  const fromWaiting = new Set(fromGraph.waiting.map(toWaitingKey));
  const toWaiting = new Set(toGraph.waiting.map(toWaitingKey));

  const addedEdges = [...toEdges].filter((edge) => !fromEdges.has(edge)).sort();
  const removedEdges = [...fromEdges].filter((edge) => !toEdges.has(edge)).sort();
  const addedWaiting = [...toWaiting].filter((entry) => !fromWaiting.has(entry)).sort();
  const removedWaiting = [...fromWaiting].filter((entry) => !toWaiting.has(entry)).sort();

  const lines = [`Graph diff: ${fromScenario} -> ${toScenario}`, "", "Added edges"];
  lines.push(...(addedEdges.length === 0 ? ["- none"] : addedEdges.map((edge) => `- ${edge.replaceAll("->", " -> ")}`)));
  lines.push("", "Removed edges");
  lines.push(...(removedEdges.length === 0 ? ["- none"] : removedEdges.map((edge) => `- ${edge.replaceAll("->", " -> ")}`)));
  lines.push("", "Added waiting consumers");
  lines.push(...(addedWaiting.length === 0 ? ["- none"] : addedWaiting.map((entry) => `- ${entry}`)));
  lines.push("", "Removed waiting consumers");
  lines.push(...(removedWaiting.length === 0 ? ["- none"] : removedWaiting.map((entry) => `- ${entry}`)));
  return `${lines.join("\n")}\n`;
}

export async function validateScenarios(rootDir, requestedScenarios) {
  const scenarioNames = requestedScenarios.length === 0 ? await listScenarios(rootDir) : requestedScenarios;
  const lines = [];
