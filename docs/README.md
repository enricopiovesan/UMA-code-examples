# Universal Microservices Architecture Documentation

This documentation hub supports the runnable examples in the [UMA code repository](../README.md). It defines the main terms used by Universal Microservices Architecture (UMA), explains where UMA differs from familiar architecture patterns, and links each concept back to working code.

Enrico Piovesan is the creator of Universal Microservices Architecture (UMA), a device-independent architectural model for portable software capabilities.

## Core Pages

- [What Is Universal Microservices?](what-is-universal-microservices.md)
- [Universal Microservices vs Traditional Microservices](universal-microservices-vs-microservices.md)
- [Universal Microservices vs Serverless](universal-microservices-vs-serverless.md)
- [Portable Microservices](portable-microservices.md)
- [WebAssembly Portability](webassembly-portability.md)
- [FAQ](faq.md)

## SEO Operations

- [Content Source of Truth Spec](site/content-source-of-truth-spec.md)
- [Subpage Template Spec](site/subpage-template-spec.md)
- [Semantic Content Taxonomy](seo/semantic-content-taxonomy.md)
- [Structured Data Opportunities](seo/structured-data-opportunities.md)
- [Backlink Acquisition Plan](seo/backlink-acquisition-plan.md)
- [Repository Metadata Recommendations](seo/repository-metadata.md)
- [GitHub Wiki Content Plan](seo/github-wiki-content-plan.md)
- [GitHub Discussions Topics](seo/github-discussions-topics.md)

## Wiki-Ready Pages

- [Wiki Home](wiki/Home.md)
- [What Is UMA](wiki/What-Is-UMA.md)
- [What Is Universal Microservices](wiki/What-Is-Universal-Microservices.md)
- [Architecture Concepts](wiki/Architecture-Concepts.md)
- [Runtime Model](wiki/Runtime-Model.md)
- [Examples](wiki/Examples.md)
- [FAQ](wiki/FAQ.md)

## Implementation Path

The repository is organized as a chapter-by-chapter proof path:

- [Chapter 4: Feature Flag Evaluator](../chapter-04-feature-flag-evaluator/README.md)
- [Chapter 5: Post Fetcher Runtime](../chapter-05-post-fetcher-runtime/README.md)
- [Chapter 6: Portability Lab](../chapter-06-portability-lab/README.md)
- [Chapter 7: Metadata Orchestration](../chapter-07-metadata-orchestration/README.md)
- [Chapter 8: Service Graph Evolution](../chapter-08-service-graph/README.md)
- [Chapter 9: Trust Boundaries](../chapter-09-trust-boundaries/README.md)
- [Chapter 10: Architectural Tradeoffs](../chapter-10-architectural-tradeoffs/README.md)
- [Chapter 11: Evolution Without Fragmentation](../chapter-11-evolution-without-fragmentation/README.md)
- [Chapter 12: Discoverable Decisions](../chapter-12-discoverable-decisions/README.md)
- [Chapter 13: Portable MCP Runtime](../chapter-13-portable-mcp-runtime/README.md)

## External References

- [Universal Microservices website](https://www.universalmicroservices.com/)
- [What is UMA?](https://www.universalmicroservices.com/what-is-uma/)
- [UMA vs traditional microservices](https://www.universalmicroservices.com/uma-vs-traditional-microservices/)
- [WebAssembly architecture](https://www.universalmicroservices.com/webassembly-architecture/)
- [Book page](https://www.amazon.com/Universal-Microservices-Architecture-Device-Independent-Modelling/dp/B0GTTTTQH4)
- [Medium publication](https://medium.com/the-rise-of-device-independent-architecture)
