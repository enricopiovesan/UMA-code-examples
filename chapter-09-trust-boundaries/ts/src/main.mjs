...
import { diffReports, evaluateTrust, formatAuditReport, listScenarios, loadScenario, validateScenarios } from "./lib.mjs";
...
async function main() {
  ...
  if (command === "render") {
    const scenarioName = process.argv[3] ?? "lab1-trusted-service";
  ...
  if (command === "diff") {
    ...
    process.stdout.write(await diffReports(rootDir, fromScenario, toScenario));
    return;
  }
