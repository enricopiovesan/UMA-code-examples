...
export async function diffReports(rootDir, fromScenario, toScenario) {
  ...
  const auditDiff = diffList(fromEntries, toEntries);

  const lines = [`Trust diff: ${fromScenario} -> ${toScenario}`, `Outcome: ${fromReport.outcome} -> ${toReport.outcome}`, "", "Added trust decisions"];
  lines.push(...(auditDiff.added.length === 0 ? ["- none"] : auditDiff.added.map((entry) => `- ${entry}`)));
  lines.push("", "Removed trust decisions");
  lines.push(...(auditDiff.removed.length === 0 ? ["- none"] : auditDiff.removed.map((entry) => `- ${entry}`)));
  return `${lines.join("\n")}\n`;
}
...
export async function validateScenarios(rootDir, requestedScenarios) {
  ...
  for (const scenarioName of scenarioNames) {
    ...
    lines.push(
      `Validated ${scenarioName}: ${report.services.length} services, ${report.auditLog.filter((entry) => entry.kind === "execution").length} executions, ${report.auditLog.filter((entry) => entry.kind === "communication").length} communications`,
    );
  }
